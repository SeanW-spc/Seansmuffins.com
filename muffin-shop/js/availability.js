...
(() => {
  ...
  function rebuildTimeOptions(selectEl, windowsList){
    ...
    const curBase = (selectEl.selectedOptions[0]?.dataset.base || selectEl.value || '').replace(/\s+—.+$/,'').trim();
    ...
    // Rebuild: keep first placeholder if present; else add one
    const hadPlaceholder = selectEl.options.length && selectEl.options[0].value === '';
    selectEl.innerHTML = '';
    if (hadPlaceholder) {
    ...
    if (curBase && windowsList.includes(curBase)) {
      ...
      // Otherwise, leave unselected; applyAvailability may auto-select if exactly one open
      selectEl.value = '';
    }
  }
