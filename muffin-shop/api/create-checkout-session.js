...
// Conditional Stripe custom field for notes (only if none typed on-site).
// Also: price preflight, optional Airtable capacity hold, address/phone collection.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const notes = (rawNotes ? String(rawNotes) : '').slice(0, 500); // keep reasonable

    // --- Stripe price preflight
    ...
    // --- Optional capacity check + pending hold (soft-fail on errors)
    ...
    // If the customer didn't enter notes on-site, show a notes field on Stripe.
    if (!notes) {
    ...
    return res.status(status).json({ error: 'CHECKOUT_CREATE_FAILED', message: msg, code });
  }
}
...
async function checkSlotAvailability(dateStr, windowStr, capacity) {
  ...
  const formula = `AND({Date}='${dateStr}', {Window}='${windowStr}', OR({Status}='confirmed', AND({Status}='pending', DATETIME_DIFF(NOW(), {Created}, 'minutes') < 60)))`;
...
async function airtableCount(url, filterByFormula) {
  ...
  do {
    ...
    const r = await fetch(query.toString(), { headers: { Authorization: `Bearer ${process.env.AIRTABLE_API_KEY}` } });
    if (!r.ok) { const t = await r.text().catch(()=> ''); throw new Error(`Airtable count error ${r.status}: ${t}`); }
...
async function createPendingReservation({ deliveryDate, timeWindow, itemsCount }) {
  ...
  if (!r.ok) { console.error('Airtable reserve failed', r.status, await r.text().catch(()=>'')); }
  return id;
}
function randomId(){ return 'res_' + Math.random().toString(36).slice(2) + Math.random().toString(36).slice(2); }
